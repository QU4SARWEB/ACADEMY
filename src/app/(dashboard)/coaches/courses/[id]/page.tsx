...
export default async function CourseDetailPage({
  ...
  const { data: course } = await supabase.from('courses').select('*, seasons(name)').eq('id', id).maybeSingle()
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-wrap gap-3">
        <Link
          href={`/coaches/courses/${id}/attendance`}
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div>
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link
              href={`/coaches/courses/${id}/modules/new`}
              className="btn-glow-sm flex items-center gap-2 rounded-lg bg-[#8B5CF6] px-3 py-1.5 text-sm text-white transition hover:bg-[#7C3AED]"
            >
              <Plus size={14} /> Añadir
            </Link>
          </div>
          ...
          <div className="space-y-2">
            {(modules ?? []).length === 0 && (
              <p className="text-sm text-zinc-500">Sin módulos todavía.</p>
            )}
            {(modules ?? []).map((mod, i) => (
              <Link
                key={mod.id}
                href={`/coaches/courses/${id}/modules/${mod.id}`}
        ...
        <div>
          <div className="mb-4 flex items-center justify-between">
            <h2 className="font-heading text-lg font-bold text-white">
              Alumnos inscritos ({enrollments?.length ?? 0})
            </h2>
          ...
          <div className="space-y-2">
            {(enrollments ?? []).length === 0 && (
              <p className="text-sm text-zinc-500">Sin alumnos inscritos.</p>
            )}
            {(enrollments ?? []).map((enr) => (
