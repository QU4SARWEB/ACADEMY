...
export async function initCoachStudentGrades(): Promise<void> {
  try {
    ...
    if (enrollIds.length > 0) {
      ...
      examGrades = (exams ?? []).map((e: any) => ({ id: '', profile_id: studentId, coach_id: '', title: 'Examen: ' + (e.exams?.title || ''), category: 'exam', score: e.score, comment: null, source: 'exam', created_at: e.created_at }))
      ...
      taskGrades = (tasks ?? []).map((t: any) => ({ id: '', profile_id: studentId, coach_id: '', title: 'Tarea: ' + (t.tasks?.title || ''), category: 'task', score: t.score, comment: null, source: 'task', created_at: t.submitted_at }))
    }
