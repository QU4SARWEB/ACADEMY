  ...
  try {
    ...
    async function renderExamList(courseId: string) {
      ...
      <div class="mb-6 flex items-center justify-between">
        <div><h1 class="font-heading text-2xl font-bold text-white">${escapeHtml(currentCourseName)}</h1><p class="mt-1 text-sm text-zinc-500">${(exams ?? []).length} exámenes</p></div>
        <button id="add-exam-btn" class="rounded-lg bg-[#8B5CF6] px-4 py-2 text-sm font-medium text-white hover:bg-[#7C3AED]">${Icon('plus', 14)} Nuevo examen</button>
      </div>
    ...
    async function bindEvents(courseId: string) {
      ...
      document.querySelectorAll('.exam-qs-btn').forEach(btn => { btn.addEventListener('click', async () => { const id = (btn as HTMLElement).closest('.exam-item')?.getAttribute('data-exam-id'); if (!id) return; qsExamId.value = id; qsModal.classList.remove('hidden'); await loadQuestions(id) }) })
      ...
      async function loadAnswers(examId: string) {
        ...
        const { data: answers } = await supabase.from('student_answers').select('*').in('attempt_id', attemptIds.length ? attemptIds : ['none'])
        ...
        ansList.innerHTML = attempts.map((att: any) => { const prof = enrollMap[att.enrollment_id] || {}; const dn = [prof.riot_id || prof.full_name, prof.social_discord].filter(Boolean).join(' | ') || 'Unknown'; return '<div class="rounded-lg border border-zinc-700 bg-zinc-900/50 p-4"><div class="flex items-center justify-between mb-2"><div class="flex items-center gap-2">' + (prof.avatar_url ? '<img src="' + escapeHtml(prof.avatar_url) + '" class="h-6 w-6 rounded-full object-cover" />' : '') + '<span class="text-sm font-medium text-white">' + escapeHtml(dn) + '</span></div><div class="flex items-center gap-2"><span class="text-xs text-zinc-400">Intento ' + att.attempt_num + '</span><span class="text-xs ' + (att.score !== null ? 'text-green-400' : 'text-yellow-400') + '">' + (att.score !== null ? att.score + '%' : 'Pendiente') + '</span></div></div>' + ((answersByAtt[att.id] || []).map((sa: any, idx: number) => { const q = qMap[sa.question_id] || {}; const opts = q.question_options || []; const isMC = q.type === 'multiple_choice' || q.type === 'true_false'; return '<div class="rounded border border-zinc-800 bg-zinc-900/30 p-2 text-xs mb-1"><span class="text-[#8B5CF6] font-medium">' + (idx + 1) + '.</span> <span class="text-white">' + escapeHtml(q.stem || '') + '</span>' + (isMC ? '<div class="mt-1 space-y-0.5">' + opts.map((o: any) => { const isSel = sa.selected_option === o.id; const isCorr = o.is_correct; let cls = 'text-zinc-500'; if (isSel && isCorr) cls = 'text-green-400'; else if (isSel && !isCorr) cls = 'text-red-400'; else if (isCorr) cls = 'text-green-400/60'; return '<div class="' + cls + '"><span>' + String.fromCharCode(65 + o.order_num) + '.</span> ' + escapeHtml(o.text) + (isSel ? ' ✓' : '') + '</div>' }).join('') + '</div>' : '<div class="mt-1"><span class="text-zinc-400">R: </span><span class="text-white">' + escapeHtml(sa.text_answer || '(sin respuesta)') + '</span></div>') + '</div>' }).join('')) + '</div>' }).join('')
      }
