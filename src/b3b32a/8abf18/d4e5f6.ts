...
export async function initCoachExamsOverview(): Promise<void> {
  try {
    ...
    async function renderExamList(courseId: string) {
      ...
      <div id="new-exam-form" class="glass rounded-xl p-6">
        ...
        <form id="create-exam-form" class="space-y-3">
          <div><label class="mb-1 block text-sm text-zinc-400">Curso</label><input type="hidden" name="course_id" id="exam-course-id" value="" /><div class="flex flex-wrap gap-2">${(courses ?? []).map((c: any) => '<button type="button" class="exam-course-btn rounded-xl border px-3 py-1.5 text-xs transition hover:border-[#8B5CF6] hover:text-white ' + (c.id === courseId ? 'bg-[#8B5CF6]/20 border-[#8B5CF6] text-white' : 'border-zinc-700 text-zinc-300 hover:text-white bg-zinc-900/50') + '" data-course-id="' + escapeHtml(c.id) + '">' + escapeHtml(c.name) + '</button>').join('')}</div></div>
          ...
          <div class="grid grid-cols-2 gap-3"><div><label class="mb-1 block text-sm text-zinc-400">Nota mínima /20</label><input name="passing_score" type="number" min="0" max="20" value="12" class="w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]"></div></div>
          ...
          <div class="grid grid-cols-2 gap-3"><div><label class="mb-1 block text-sm text-zinc-400">Tipo</label><select name="eval_type" class="w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]"><option value="exam">Examen</option><option value="quiz">Quiz</option></select></div><div><label class="mb-1 block text-sm text-zinc-400">Mes</label><select name="month" class="w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]"><option value="">— Sin mes —</option>${['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'].map((n, i) => '<option value="' + (i+1) + '">' + n + '</option>').join('')}</select></div></div>
          ...
          <div class="flex items-center gap-4">${renderToggle({ name: 'is_published', label: 'Publicado', checked: true })}${renderToggle({ name: 'shuffle', label: 'Aleatorio', checked: true })}${renderToggle({ name: 'is_active', label: 'Activo', checked: true })}</div>
          ...
          <button type="submit" class="btn-glow flex items-center gap-2 rounded-lg bg-[#8B5CF6] px-4 py-2 text-sm font-medium text-white hover:bg-[#7C3AED]">${Icon('plus', 14)} Crear examen${'<span id="exam-q-count" class="ml-1 text-xs opacity-70"></span>'}</button>
        </form>
      </div>
      </div>
      ...
      <div class="space-y-3">${(exams ?? []).length === 0 ? '<p class="text-sm text-zinc-500">No hay exámenes en este curso.</p>' : (exams ?? []).map((exam: any) => '<div class="exam-item glass rounded-xl p-4" data-exam-id="' + exam.id + '" data-course-id="' + (exam.course_id || '') + '"><div class="flex items-start justify-between"><div class="min-w-0 flex-1"><div class="flex items-center gap-2"><h3 class="font-medium text-white">' + escapeHtml(exam.title) + '</h3>' + (exam.is_published ? '<span class="rounded-full bg-green-500/20 px-2 py-0.5 text-xs text-green-400">Publicado</span>' : '<span class="rounded-full bg-yellow-500/20 px-2 py-0.5 text-xs text-yellow-400">Borrador</span>') + '</div><p class="mt-1 text-xs text-zinc-500">' + 'Nota mín: ' + exam.passing_score + '% · Tiempo: ' + (exam.time_limit || 300) + 'min' + (exam.max_attempts ? ' · Intentos: ' + exam.max_attempts : '') + (exam.due_date ? ' · Vence: ' + formatDate(exam.due_date) : '') + '</p></div><div class="flex gap-2 shrink-0 ml-3"><button class="exam-answers-btn rounded-lg border border-zinc-700 px-3 py-1.5 text-xs text-zinc-300 hover:bg-zinc-800">' + Icon('users', 12) + ' Respuestas</button><button class="edit-exam-btn rounded-lg border border-zinc-700 px-3 py-1.5 text-xs text-zinc-300 hover:bg-zinc-800">' + Icon('edit', 12) + '</button><button class="del-exam-btn rounded-lg border border-red-700 px-3 py-1.5 text-xs text-red-400 hover:bg-red-900/30">' + Icon('trash', 12) + '</button></div></div><div class="edit-exam-form mt-3 hidden border-t border-zinc-700 pt-3"><form data-exam-id="' + exam.id + '"><div class="grid grid-cols-2 gap-3 mb-3"><div><label class="text-xs text-zinc-400">Título</label><input name="title" value="' + escapeHtml(exam.title) + '" class="w-full rounded border border-zinc-700 bg-zinc-900 px-3 py-1.5 text-sm text-white focus:border-[#8B5CF6]"></div></div><div class="grid grid-cols-3 gap-3 mb-3"><div><label class="text-xs text-zinc-400">Nota mín %</label><input name="passing_score" type="number" value="' + exam.passing_score + '" class="w-full rounded border border-zinc-700 bg-zinc-900 px-3 py-1.5 text-sm text-white focus:border-[#8B5CF6]"></div><div><label class="text-xs text-zinc-400">Tiempo (min)</label><input name="time_limit" type="number" value="' + (exam.time_limit || '') + '" class="w-full rounded border border-zinc-700 bg-zinc-900 px-3 py-1.5 text-sm text-white focus:border-[#8B5CF6]"></div><div><label class="text-xs text-zinc-400">Intentos</label><input name="max_attempts" type="number" value="' + exam.max_attempts + '" class="w-full rounded border border-zinc-700 bg-zinc-900 px-3 py-1.5 text-sm text-white focus:border-[#8B5CF6]"></div></div><div class="mb-3"><label class="text-xs text-zinc-400">Descripción</label><textarea name="description" rows="2" class="w-full rounded border border-zinc-700 bg-zinc-900 px-3 py-1.5 text-sm text-white focus:border-[#8B5CF6]">' + escapeHtml(exam.description || '') + '</textarea></div><div class="mb-3 flex items-center gap-3">' + renderToggle({ name: 'is_published', label: 'Publicado', checked: exam.is_published !== false }) + renderToggle({ name: 'shuffle', label: 'Aleatorio', checked: exam.shuffle !== false }) + renderToggle({ name: 'is_active', label: 'Activo', checked: exam.is_active !== false }) + '</div><div class="flex gap-2"><button type="submit" class="rounded-lg bg-[#8B5CF6] px-3 py-1.5 text-xs font-medium text-white">Guardar</button><button type="button" class="cancel-edit-btn rounded-lg border border-zinc-700 px-3 py-1.5 text-xs text-zinc-300">Cancelar</button></div></form></div></div>').join('')}</div></div></div></div>` + modalsHtml
    ...
    async function bindEvents(courseId: string) {
      ...
      document.querySelectorAll('.exam-course-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          document.getElementById('exam-course-id')!.setAttribute('value', (btn as HTMLElement).dataset.courseId || '')
        })
      })
      ...
      document.getElementById('create-exam-form')?.addEventListener('submit', async (e) => {
        ...
        const payload: Record<string, any> = {
          ...
          passing_score: parseFloat(fd.get('passing_score') as string) || 12, time_limit: parseInt(fd.get('time_limit') as string) || null,
          max_attempts: parseInt(fd.get('max_attempts') as string) || 1, weight: parseFloat(fd.get('weight') as string) || 0,
          due_date: fd.get('due_date') || null, is_published: !!fd.get('is_published'), shuffle: !!fd.get('shuffle'),
          eval_type: (fd.get('eval_type') as string) || 'exam', month: fd.get('month') ? parseInt(fd.get('month') as string) : null, is_active: !!fd.get('is_active'),
        }
        ...
        document.getElementById('new-exam-form')?.classList.add('hidden'); ;(window as any).__blockReload = false; renderExamList(courseId)
        } catch (err) { console.error(err); toast('error', 'Error al crear examen'); ;(window as any).__blockReload = false
      ...
      document.querySelectorAll('.edit-exam-form form').forEach(form => {
        form.addEventListener('submit', async (e) => { e.preventDefault(); const fd = new FormData(e.target as HTMLFormElement); const examId = (e.target as HTMLElement).dataset.examId; if (!examId) return; const { error } = await supabase.from('exams').update({ title: fd.get('title'), description: fd.get('description'), passing_score: parseFloat(fd.get('passing_score') as string) || 12, time_limit: parseInt(fd.get('time_limit') as string) || null, max_attempts: parseInt(fd.get('max_attempts') as string) || 1, is_published: !!fd.get('is_published'), shuffle: !!fd.get('shuffle'), is_active: !!fd.get('is_active') }).eq('id', examId); if (error) { toast('error', error.message); return }; toast('success', 'Examen actualizado'); renderExamList(courseId) })
      })
      ...
      async function loadAnswers(examId: string) {
        ...
        const { data: exam } = await supabase.from('exams').select('course_id').eq('id', examId).maybeSingle()
        const courseIdForExam = exam?.course_id || document.querySelector<HTMLElement>('.exam-item[data-exam-id="' + examId + '"]')?.getAttribute('data-course-id')
        const { data: enrolls } = await supabase.from('enrollments').select('id, profile_id, profiles!profile_id(full_name, avatar_url, riot_id, social_discord)').eq('course_id', courseIdForExam || '').eq('status', 'active')
        const { data: attempts } = await supabase.from('exam_attempts').select('*').eq('exam_id', examId)
        const attemptByEnroll: Record<string, any> = {}
        for (const a of attempts ?? []) { if (!attemptByEnroll[a.enrollment_id] || a.status !== 'in_progress') attemptByEnroll[a.enrollment_id] = a }
        ansList.innerHTML = (enrolls ?? []).length === 0 ? '<p class="text-sm text-zinc-500">No hay alumnos inscritos en este curso.</p>' : (enrolls ?? []).map((enr: any) => {
          const p: any = enr.profiles || {}; const dn = [p.riot_id || p.full_name, p.social_discord].filter(Boolean).join(' | ') || p.full_name || 'Unknown'
          const att = attemptByEnroll[enr.id]
          const statusColor = att ? (att.score !== null ? (att.score >= 14 ? 'text-green-400' : 'text-red-400') : 'text-yellow-400') : 'text-zinc-600'
          const statusText = att ? (att.score !== null ? att.score + '/20' : 'Pendiente') : 'No ha rendido'
          return '<div class="exam-attempt-item rounded-lg border border-zinc-700 bg-zinc-900/50 p-4 ' + (att ? 'cursor-pointer hover:bg-zinc-800/50' : 'opacity-70') + ' transition" data-attempt-id="' + (att?.id || '') + '"><div class="flex items-center justify-between"><div class="flex items-center gap-3">' + (p.avatar_url ? '<img src="' + escapeHtml(p.avatar_url) + '" class="h-8 w-8 rounded-full object-cover" />' : '<div class="flex h-8 w-8 items-center justify-center rounded-full bg-[#8B5CF6]/20 text-xs font-bold text-[#8B5CF6]">' + escapeHtml(dn.charAt(0)) + '</div>') + '<div><p class="text-sm font-medium text-white' + (att ? ' hover:text-[#8B5CF6]' : '') + ' transition">' + escapeHtml(dn) + '</p><p class="text-xs text-zinc-500">' + (att ? 'Intento ' + att.attempt_num : '') + '</p></div></div><div class="flex items-center gap-2"><span class="text-sm font-semibold ' + statusColor + '">' + statusText + '</span>' + (att ? '<span class="text-zinc-600">' + Icon('chevronRight', 16) + '</span>' : '') + '</div></div></div>'
        }).join('')
        document.querySelectorAll('.exam-attempt-item').forEach(el => { el.addEventListener('click', () => { const attId = el.getAttribute('data-attempt-id');           if (attId) { ansModal.classList.add('hidden'); location.hash = '#/coaches/exams/attempt/' + examId + '/' + attId } }) })
      }
