    ...
    async function renderExamList(courseId: string) {
      ...
      <div id="new-exam-form" class="glass rounded-xl p-6">
        ...
        <form id="create-exam-form" class="space-y-3">
          ...
          <div class="grid grid-cols-2 gap-3"><div><label class="mb-1 block text-sm text-zinc-400">Nota mínima %</label><input name="passing_score" type="number" min="0" max="100" value="60" class="w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-white outline-none focus:border-[#8B5CF6]"></div></div>
    ...
    async function bindEvents(courseId: string) {
      ...
      async function loadAnswers(examId: string) {
        ...
        ansList.innerHTML = (enrolls ?? []).length === 0 ? '<p class="text-sm text-zinc-500">No hay alumnos inscritos en este curso.</p>' : (enrolls ?? []).map((enr: any) => {
          ...
          const statusColor = att ? (att.score !== null ? (att.score >= 70 ? 'text-green-400' : 'text-red-400') : 'text-yellow-400') : 'text-zinc-600'
          const statusText = att ? (att.score !== null ? att.score + '%' : 'Pendiente') : 'No ha rendido'
