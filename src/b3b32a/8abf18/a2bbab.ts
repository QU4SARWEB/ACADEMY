    ...
    const html = `
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody>
            ${(players ?? []).length === 0
              ? '<tr><td colspan="10" class="pt-4 text-zinc-500">No hay jugadores registrados.</td></tr>'
              : (players ?? []).map((p: any) => {
                  ...
                  return `
                    <tr class="border-b border-zinc-800/50">
                      <td class="py-3 pr-2"><input type="checkbox" class="row-checkbox h-4 w-4 rounded border-zinc-700 bg-zinc-900 text-[#8B5CF6]" value="${escapeHtml(p.id)}"></td>
                      <td class="py-3 pr-4">
                        <div class="flex items-center gap-2">
                          ...
                          <a href="#/coaches/players/${escapeHtml(p.id)}" class="text-white hover:text-[#8B5CF6] transition">${escapeHtml(displayName)}</a>
                        </div>
                      </td>
                      ...
                      <td class="py-3 pr-4"><div class="flex flex-wrap gap-1">${courseNames.length > 0 ? courseNames.map((n: string) => '<span class="rounded bg-zinc-800 px-1.5 py-0.5 text-[10px] text-zinc-400">' + escapeHtml(n) + '</span>').join('') : '<span class="text-xs text-zinc-600">—</span>'}</div></td>
                      <td class="py-3 pr-4"><span class="text-xs ${payColor}">${payStatus === 'gratis' ? 'Gratis' : escapeHtml(payStatus)}</span></td>
