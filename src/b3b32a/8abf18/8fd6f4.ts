...
async function loadTeamsData() {
  ...
  return { teams: teams ?? [], membersByTeam, allProfiles: allProfiles ?? [] }
}

function renderTeamCards(teams: any[], membersByTeam: Record<string, any[]>, allProfiles: any[]): string {
...
function renderNewTeamForm(): string {
  return `
...
function renderBulkAddModal(): string {
  return `
...
function initTeamEventHandlers(container: HTMLElement, teams: any[], membersByTeam: Record<string, any[]>, allProfiles: any[]): void {
...
export async function initCoachTeams(): Promise<void> {
  try {
    const { teams, membersByTeam, allProfiles } = await loadTeamsData()
    ...
    document.getElementById('modal-root')!.insertAdjacentHTML('beforeend', renderEditTeamModal())
    document.getElementById('modal-root')!.insertAdjacentHTML('beforeend', renderBulkAddModal())

    initTeamEventHandlers(container, teams, membersByTeam, allProfiles)
  } catch (err) {
