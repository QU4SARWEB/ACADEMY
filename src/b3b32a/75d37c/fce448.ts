...
export async function initStudentGrades(): Promise<void> {
  try {
    ...
    if (enrollIds.length > 0) {
      const { data: exams } = await supabase.from('exam_attempts').select('score, exams!inner(title), created_at').in('enrollment_id', enrollIds).not('score', 'is', null).order('created_at', { ascending: false })
      examGrades = (exams ?? []).map((e: any) => ({ title: 'Examen: ' + (e.exams?.title || ''), category: 'exam', score: e.score, created_at: e.created_at }))
    ...
    const html = `
      ...
      <div class="mb-6">
        ...
        <div class="space-y-3">
          ${(enrolls ?? []).length === 0 ? '<p class="text-sm text-zinc-500">No hay cursos activos.</p>' : (enrolls ?? []).map((e: any) => '<div class="glass rounded-xl p-4"><div class="flex items-center justify-between"><div><h3 class="font-medium text-white">' + escapeHtml(e.courses?.name || 'Sin curso') + '</h3></div><div class="text-right"><p class="text-lg font-bold ' + ((e.final_grade ?? 0) >= 70 ? 'text-green-400' : 'text-yellow-400') + '">' + (e.final_grade !== null ? e.final_grade + '%' : '-') + '</p><p class="text-xs text-zinc-500">Nota final</p></div></div>' + (e.promoted ? '<span class="mt-2 inline-block text-xs text-green-400">Promovido</span>' : '') + '</div>').join('')}
        </div>
      </div>
